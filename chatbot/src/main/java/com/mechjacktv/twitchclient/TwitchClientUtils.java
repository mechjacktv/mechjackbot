...
final class TwitchClientUtils {
    ...
    final URLConnection openConnection(final String serviceUrl) {
    ...
    final InputStream openResponseInputStream(final String serviceUrl) {
        return this.executionUtils.softenException(() -> this.openConnection(serviceUrl).getInputStream(),
                ConnectionException.class);
    }

    final Reader openResponseReader(final String serviceUrl) {
