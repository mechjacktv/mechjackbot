 ...
 * this positional parameter if no value was specified on the command line
 * @since 3.2
 */
String defaultValue()default"__no_default_value__";
 ...
 * not check for missing required options and positional parameters on the
 * parent command
 * @since 3.0
 */
...
public interface IDefaultValueProvider {
   ...
   * {@code null} if this provider has no default value for the specified option
...
public interface IFactory {
  ...
  public static class Range implements Comparable<Range> {
    ...
    public Range min(int newMin) {
      return new Range(newMin, Math.max(newMin, this.max), this.isVariable, this.isUnspecified,
          this.originalValue);
    }
    ...
    public Range max(int newMax) {
      return new Range(Math.min(this.min, newMax), newMax, this.isVariable, this.isUnspecified,
          this.originalValue);
    }
    ...
    public Range unspecified(boolean unspecified) {
      return new Range(this.min, this.max, this.isVariable, unspecified,
          this.originalValue);
    }
  ...
  public static final class Model {
    ...
    public static class CommandSpec {
       ...
       * {@linkplain Option#converter() converters}, etc., that are registered
       * declaratively with annotation attributes
       * @throws InitializationException if the specified object has no picocli
       * annotations or has invalid annotations
       */
      ...
      void validate() {
        ...
        if (!wrongVersionHelpAttr.isEmpty()) {
          throw new InitializationException(
              String.format(wrongType, wrongVersionHelpAttr, "versionHelp", "--version", "version information"));
        }
      ...
      public CommandSpec addSubcommand(String name, CommandLine subCommandLine) {
        ...
        if (previous != null && previous != subCommandLine) {
          throw new InitializationException(
              "Another subcommand named '" + name + "' already exists for command '" + this.name() + "'");
        }
        ...
        for (String alias : subSpec.aliases()) {
          ...
          if (previous != null && previous != subCommandLine) {
            throw new InitializationException("Alias '" + alias + "' for subcommand '" + name
       ...
       * converters, etc., that are registered declaratively with annotation
       * attributes
       * @return this {@link CommandSpec} object for method chaining
       * @see #addSubcommand(String, CommandLine)
       * @since 3.7.0
      ...
      public boolean mixinStandardHelpOptions() {
        return this.mixins.containsKey(AutoHelpMixin.KEY);
      }
       ...
       * strings, or {@code null} if the {@linkplain #version(String...) version
      ...
      void initName(String value) {
        if (initializable(this.name, value, DEFAULT_COMMAND_NAME)) {
          this.name = value;
        }
      }

      void initHelpCommand(boolean value) {
        if (initializable(this.isHelpCommand, value, DEFAULT_IS_HELP_COMMAND)) {
          this.isHelpCommand = value;
        }
      }

      void initVersion(String[] value) {
        if (initializable(this.version, value, UsageMessageSpec.DEFAULT_MULTI_LINE)) {
      ...
      public OptionSpec findOption(char shortName) {
        return findOption(shortName, this.options());
      }
      ...
      public OptionSpec findOption(String name) {
        return findOption(name, this.options());
      }
      ...
      boolean resemblesOption(String arg, Tracer tracer) {
        ...
        boolean result = count > 0 && count * 10 >= this.optionsMap().size() * 9; // at least one prefix char in common
                                                                                  // with 9 out of 10 options
        if (tracer != null && tracer.isDebug()) {
          tracer.debug("%s %s an option: %d matching prefix chars out of %d option names%n", arg,
              (result ? "resembles" : "doesn't resemble"), count,
    ...
    public static class UsageMessageSpec {
      ...
      private static int getSysPropertyWidthOrDefault(int defaultWidth) {
        ...
        try {
          ...
          if (width < MINIMUM_USAGE_WIDTH) {
            new Tracer().warn("Invalid picocli.usage.width value %d. Using minimum usage width %d.%n", width,
                MINIMUM_USAGE_WIDTH);
      ...
      public String parameterListHeading() {
        return this.str(this.resourceStr("usage.parameterListHeading"),
      ...
      public char requiredOptionMarker() {
        return (this.requiredOptionMarker == null) ? DEFAULT_REQUIRED_OPTION_MARKER : this.requiredOptionMarker;
      }
      ...
      public UsageMessageSpec requiredOptionMarker(char newValue) {
        this.requiredOptionMarker = newValue;
        return this;
      }
    ...
    public static class ParserSpec {
      ...
      void initSeparator(String value) {
        if (initializable(this.separator, value, DEFAULT_SEPARATOR)) {
      ...
      public String toString() {
        return String.format("posixClusteredShortOptionsAllowed=%s, stopAtPositional=%s, stopAtUnmatched=%s, " +
            ...
            this.atFileCommentChar, this.endOfOptionsDelimiter, this.limitSplit,
            this.aritySatisfiedByAttachedOptionParam,
    ...
    public abstract static class ArgSpec {
      ...
      private <T extends Builder<T>> ArgSpec(Builder<T> builder) {
        ...
        if (builder.auxiliaryTypes == null || builder.auxiliaryTypes.length == 0) {
          ...
          } else if (Collection.class.isAssignableFrom(this.type)) { // type is a collection but element type is
                                                                     // unspecified
      ...
      public Class<?>[] auxiliaryTypes() {
        return this.auxiliaryTypes.clone();
      }
       ...
       * parameter, returning {@code null} means this option or positional parameter
      ...
      public <T> T setValue(T newValue, CommandLine commandLine) throws PicocliException {
        if (this.setter instanceof MethodBinding) {
          ((MethodBinding) this.setter).commandLine = commandLine;
        }
        try {
          return this.setter.set(newValue);
        } catch (PicocliException ex) {
       ...
       * before type conversion. For map properties, {@code "key=value"} values are
       ...
       * options without value, the values have not been {@linkplain #splitRegex()
       * split}, and for map properties values may look like {@code "key=value"}
       */
      ...
      private String[] debug(String[] result, String msg, String value) {
        Tracer t = new Tracer();
        if (t.isDebug()) {
          t.debug("%s with regex '%s' resulted in %s parts: %s%n", msg, this.splitRegex(), result.length,
      ...
      private String[] splitRespectingQuotedStrings(String value, int limit, ParserSpec parser) {
        ...
        if (!quotedValues.isEmpty()) {
          new Tracer().warn("Unable to respect quotes while splitting value %s for %s (unprocessed remainder: %s)%n",
      ...
      abstract static class Builder<T extends Builder<T>> {
         ...
         * options and positional parameters this can be an array, or a (sub-type of)
         * Collection or Map.
         */
    ...
    public static class OptionSpec extends ArgSpec {
      ...
      protected boolean internalShowDefaultValue(boolean usageMessageShowDefaults) {
        return super.internalShowDefaultValue(usageMessageShowDefaults) && !this.help() && !this.versionHelp()
            && !this.usageHelp();
      }
      ...
      public String longestName() {
        return Help.ShortestFirst.longestFirst(this.names.clone())[0];
      }

      /** Returns the shortest {@linkplain #names() option name}. */
      String shortestName() {
        return Help.ShortestFirst.sort(this.names.clone())[0];
      }
          ...
          try {
            ...
            throw new PicocliException(
                String.format("Could not invoke setter (%s) with unmatched argument array '%s': %s",
                    this.setter, Arrays.toString(unmatched), ex),
                ex);
          }
        }
        if (this.getter != null) {
          try {
            ...
            throw new PicocliException(
                String.format("Could not add unmatched argument array '%s' to collection returned by getter (%s): %s",
                    Arrays.toString(unmatched), this.getter, ex),
                ex);
          }
        }
      }
    ...
    static class TypedMember {
      ...
      String toGenericString() {
        return this.accessible instanceof Field ? ((Field) this.accessible).toGenericString()
            : this.accessible instanceof Method ? ((Method) this.accessible).toGenericString()
                : this.accessible
                    .toString();
      }
    ...
    public static class Messages {
       ...
       * find a value by qualifying the key with the command's fully qualified name,
      ...
      public String getString(String key, String defaultValue) {
        if (this.rb == null || this.keys.isEmpty()) {
          return defaultValue;
        }
    ...
    private static class CommandReflection {
      ...
      private static boolean updateCommandAttributes(Class<?> cls, CommandSpec commandSpec, IFactory factory) {
        // superclass values should not overwrite values if both class and superclass
        // have a @Command annotation
        if (!cls.isAnnotationPresent(Command.class)) {
          return false;
        }
      ...
      private static void initSubcommands(Command cmd, CommandSpec parent, IFactory factory) {
        for (Class<?> sub : cmd.subcommands()) {
          try {
            if (Help.class == sub) {
              throw new InitializationException(Help.class.getName() + " is not a valid subcommand. Did you mean "
      ...
      private static boolean initFromMethodParameters(Object scope, Method method, CommandSpec receiver,
          IFactory factory) {
        boolean result = false;
        int optionCount = 0;
      ...
      private static void validateMixin(TypedMember member) {
        if (member.isMixin() && member.isArgSpec()) {
          throw new DuplicateOptionAnnotationsException(
              "A member cannot be both a @Mixin command and an @Option or @Parameters, but '" + member + "' is both.");
        }
        if (member.isMixin() && member.isUnmatched()) {
          throw new DuplicateOptionAnnotationsException(
              "A member cannot be both a @Mixin command and an @Unmatched but '" + member + "' is both.");
        }
      }
      ...
      private static void validateUnmatched(TypedMember member) {
        if (member.isUnmatched() && member.isArgSpec()) {
          throw new DuplicateOptionAnnotationsException(
              "A member cannot have both @Unmatched and @Option or @Parameters annotations, but '" + member
                  + "' has both.");
        }
      }
      ...
      private static void validateArgSpecField(TypedMember member) {
        ...
        if (Modifier.isFinal(field.getModifiers())
          ...
          throw new InitializationException(
              "Constant (final) primitive and String fields like " + field + " cannot be used as " +
      ...
      private static void validateInjectSpec(TypedMember member) {
        if (member.isInjectSpec() && (member.isOption() || member.isParameter())) {
          throw new DuplicateOptionAnnotationsException(
              "A member cannot have both @Spec and @Option or @Parameters annotations, but '" + member + "' has both.");
        }
        if (member.isInjectSpec() && member.isUnmatched()) {
          throw new DuplicateOptionAnnotationsException(
              "A member cannot have both @Spec and @Unmatched annotations, but '" + member + "' has both.");
        }
        if (member.isInjectSpec() && member.isMixin()) {
          throw new DuplicateOptionAnnotationsException(
              "A member cannot have both @Spec and @Mixin annotations, but '" + member + "' has both.");
        }
        if (member.getType() != CommandSpec.class) {
          throw new InitializationException("@picocli.CommandLine.Spec annotation is only supported on fields of type "
    ...
    static class ArgsReflection {
      ...
      static PositionalParamSpec extractPositionalParamSpec(TypedMember member, IFactory factory) {
        ...
        if (!NoCompletionCandidates.class.equals(parameters.completionCandidates())) {
          builder.completionCandidates(
              DefaultFactory.createCompletionCandidates(factory, parameters.completionCandidates()));
        }
        return builder.build();
      }
      ...
      private static void initCommon(ArgSpec.Builder<?> builder, TypedMember member) {
        ...
        builder.withToString((member.accessible instanceof Field ? "field "
            : member.accessible instanceof Method ? "method " : member.accessible.getClass().getSimpleName() + " ")
    ...
    private static class FieldBinding implements IGetter, ISetter {
      ...
      public <T> T get() throws PicocliException {
        try {
          @SuppressWarnings("unchecked")
          T result = (T) this.field.get(this.scope);
          return result;
        } catch (Exception ex) {
    ...
    static class MethodBinding implements IGetter, ISetter {
      ...
      public <T> T set(T value) throws PicocliException {
        try {
          ...
          throw new ParameterException(this.commandLine, "Could not invoke " + this.method + " with " + value,
              ex.getCause());
        } catch (Exception ex) {
  ...
  public static class ParseResult {
    ...
    public static class Builder {
       ...
       * {@code PositionalParamSpec} was matched. Ignored for {@code OptionSpec}s.
    ...
    public OptionSpec matchedOption(char shortName) {
      return CommandSpec.findOption(shortName, this.matchedOptions);
    }
     ...
     * name that was actually specified on the command line. The specified name may
     * include option name prefix characters or not.
     */
    public OptionSpec matchedOption(String name) {
      return CommandSpec.findOption(name, this.matchedOptions);
    }
    ...
    public <T> T matchedPositionalValue(int position, T defaultValue) {
      return this.matchedPositionalValue(
...
private class Interpreter {
  ...
  private void registerBuiltInConverters() {
    ...
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.sql.Connection",
        "java.sql.DriverManager", "getConnection", String.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.sql.Driver",
        "java.sql.DriverManager", "getDriver", String.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.sql.Timestamp",
        "java.sql.Timestamp", "valueOf", String.class);

    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.Duration", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.Instant", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.LocalDate", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.LocalDateTime", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.LocalTime", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.MonthDay", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.OffsetDateTime", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.OffsetTime", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.Period", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.Year", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.YearMonth", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.ZonedDateTime", "parse",
        CharSequence.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.ZoneId", "of",
        String.class);
    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.time.ZoneOffset", "of",
        String.class);

    BuiltIn.registerIfAvailable(this.converterRegistry, CommandLine.this.tracer, "java.nio.file.Path",
  ...
  List<CommandLine> parse(String... args) {
    ...
    if (CommandLine.this.tracer.isDebug()) {
      CommandLine.this.tracer.debug(
          "(ANSI is %s by default: TTY=%s, isXTERM=%s, hasOSTYPE=%s, isWindows=%s, JansiConsoleInstalled=%s)%n",
          Help.Ansi.ansiPossible() ? "enabled" : "disabled", Help.Ansi.isTTY(), Help.Ansi.isXterm, Help.Ansi.hasOsType,
          Help.Ansi.isWindows, Help.Ansi.isJansiConsoleInstalled());
    }
  ...
  private void clear(ArgSpec argSpec) {
    ...
    if (argSpec.hasInitialValue()) {
      try {
        ...
        CommandLine.this.tracer.debug("Set initial value for %s of type %s to %s.%n", argSpec, argSpec.type(),
            String.valueOf(argSpec.initialValue()));
      } catch (Exception ex) {
        CommandLine.this.tracer
            .warn("Could not set initial value for %s of type %s to %s: %s%n", argSpec, argSpec.type(),
                String.valueOf(argSpec.initialValue()), ex);
      }
    } else {
  ...
  private void parse(List<CommandLine> parsedCommands, Stack<String> argumentStack, String[] originalArgs,
    ...
    if (CommandLine.this.tracer.isDebug()) {
      CommandLine.this.tracer.debug(
          ...
          CommandLine.this.commandSpec.toString(),
          new HashSet<ArgSpec>(CommandLine.this.commandSpec.optionsMap().values()).size(),
          CommandLine.this.commandSpec.positionalParameters().size(),
          CommandLine.this.commandSpec.requiredArgs().size(), CommandLine.this.commandSpec
              .subcommands().size());
    }
  ...
  private void processArguments(List<CommandLine> parsedCommands,
    ...
    while (!args.isEmpty()) {
      ...
      if (CommandLine.this.commandSpec.parser.endOfOptionsDelimiter().equals(arg)) {
        CommandLine.this.tracer
            .info("Found end-of-options delimiter '--'. Treating remainder as positional parameters.%n");
      ...
      if (separatorIndex > 0) {
        ...
        if (CommandLine.this.commandSpec.optionsMap().containsKey(key)
            && !CommandLine.this.commandSpec.optionsMap().containsKey(arg)) {
  ...
  private void processPositionalParameter(Collection<ArgSpec> required, Set<ArgSpec> initialized, Stack<String> args)
      throws Exception {
    if (CommandLine.this.tracer.isDebug()) {
    ...
    for (PositionalParamSpec positionalParam : CommandLine.this.commandSpec.positionalParameters()) {
      ...
      if (!this.assertNoMissingParameters(positionalParam, arity, argsCopy)) {
        break;
      } // #389 collectErrors parsing
      int originalSize = argsCopy.size();
      int actuallyConsumed = this
          .applyOption(positionalParam, LookBehind.SEPARATE, arity, argsCopy, initialized,
              "args[" + indexRange + "] at position " + this.position);
    ...
    if (CommandLine.this.tracer.isDebug()) {
      CommandLine.this.tracer
          .debug("Consumed %d arguments and %d interactive values, moving position to index %d.%n", argsConsumed,
              interactiveConsumed,
              this.position);
    }
  ...
  private int applyOption(ArgSpec argSpec,
    ...
    boolean consumeOnlyOne = CommandLine.this.commandSpec.parser().aritySatisfiedByAttachedOptionParam()
        && lookBehind.isAttached();
    Stack<String> workingStack = args;
    if (consumeOnlyOne) {
      workingStack = args.isEmpty() ? args : this.stack(args.pop());
    } else {
      if (!this.assertNoMissingParameters(argSpec, arity, args)) {
        return 0;
      } // #389 collectErrors parsing
    }
  ...
  private int applyValueToSingleValuedField(ArgSpec argSpec,
    ...
    Class<?> cls = argSpec.auxiliaryTypes()[0]; // field may be interface/abstract type, use annotation to get concrete
                                                // type
    if (arity.min <= 0) { // value may be optional
      ...
      if (cls == Boolean.class || cls == Boolean.TYPE) {
        ...
        } else if (lookBehind != LookBehind.ATTACHED_WITH_SEPARATOR) { // if attached, try converting the value to
                                                                       // boolean (and fail if invalid value)
      ...
      if (initialized.contains(argSpec)) {
        if (!CommandLine.this.isOverwrittenOptionsAllowed()) {
          throw new OverwrittenOptionException(CommandLine.this, argSpec,
              this.optionDescription("", argSpec, 0) + " should be specified only once");
        }
    ...
    this.parseResult.addOriginalStringValue(argSpec, value);// #279 track empty string value if no command line argument
                                                            // was consumed
  ...
  private int applyValuesToMapField(ArgSpec argSpec,
    ...
    if (map == null || (!map.isEmpty() && !initialized.contains(argSpec))) {
      CommandLine.this.tracer
          .debug("Initializing binding for %s with empty %s%n", this.optionDescription("", argSpec, 0),
              argSpec.type().getSimpleName());
      map = this.createMap(argSpec.type()); // map class
      argSpec.setValue(map, CommandLine.this.commandSpec.commandLine());
    }
    initialized.add(argSpec);
    int originalSize = map.size();
    this.consumeMapArguments(argSpec, lookBehind, arity, args, classes, keyConverter, valueConverter, map,
        argDescription);
    this.parseResult.add(argSpec, this.position);
    argSpec.setValue(map, CommandLine.this.commandSpec.commandLine());
  ...
  private void consumeMapArguments(ArgSpec argSpec,
    ...
    for (int i = 0; consumed < arity.min && !args.isEmpty(); i++) {
      ...
      this.consumeOneMapArgument(argSpec, lookBehind, arity, consumed, args.pop(), classes, keyConverter,
          valueConverter, typedValuesAtPosition, i, argDescription);
    ...
    for (int i = consumed; consumed < arity.max && !args.isEmpty(); i++) {
      ...
      Map<Object, Object> typedValuesAtPosition = new LinkedHashMap<Object, Object>();
      this.parseResult.addTypedValues(argSpec, currentPosition++, typedValuesAtPosition);
      if (!this.canConsumeOneMapArgument(argSpec, arity, consumed, args.peek(), classes, keyConverter, valueConverter,
          argDescription)) {
  ...
  private void consumeOneMapArgument(ArgSpec argSpec,
    ...
    for (String value : values) {
      ...
      if (CommandLine.this.tracer.isInfo()) {
        CommandLine.this.tracer.info("Putting [%s : %s] in %s<%s, %s> %s for %s%n", String.valueOf(mapKey),
            String.valueOf(mapValue),
            result.getClass().getSimpleName(), classes[0].getSimpleName(), classes[1].getSimpleName(),
            argSpec.toString(), argDescription);
      }
  ...
  private String[] splitKeyValue(ArgSpec argSpec, String value) {
    ...
    if (keyValue.length < 2) {
      ...
      if (splitRegex.length() == 0) {
        ...
        throw new ParameterException(CommandLine.this, "Value for option " + this.optionDescription("",
            argSpec, 0) + " should be in KEY=VALUE[" + splitRegex + "KEY=VALUE]... format but was " + value, argSpec,
            value);
      }
    }
    return keyValue;
  ...
  private List<Object> consumeArguments(ArgSpec argSpec,
    ...
    for (int i = 0; consumed < arity.min && !args.isEmpty(); i++) {
      ...
      this.parseResult.addTypedValues(argSpec, currentPosition++, typedValuesAtPosition);
      this.assertNoMissingMandatoryParameter(argSpec, args, i, arity);
      this.consumeOneArgument(argSpec, lookBehind, arity, consumed, args.pop(), type, typedValuesAtPosition, i,
          argDescription);
  ...
  private int consumeOneArgument(ArgSpec argSpec,
    ...
    for (int j = 0; j < values.length; j++) {
      ...
      if (CommandLine.this.tracer.isInfo()) {
        CommandLine.this.tracer
            .info("Adding [%s] to %s for %s%n", String.valueOf(result.get(result.size() - 1)), argSpec.toString(),
                argDescription);
      }
  ...
  private boolean canConsumeOneArgument(ArgSpec argSpec, Range arity, int consumed, String arg, Class<?> type,
      String argDescription) {
    ITypeConverter<?> converter = this.getTypeConverter(type, argSpec, 0);
    try {
      ...
      // tracer.debug("$s would split into %s values but %s cannot accept that many
      // values.%n", arg, values.length, argDescription);
      // return false;
      // }
      for (String value : values) {
  ...
  private boolean isOption(String arg) {
    ...
    if (CommandLine.this.commandSpec.optionsMap().containsKey(arg)) { // -v or -f or --file (not attached to param or
                                                                      // other option)
      return true;
    }
    ...
    return (arg.length() > 2 && arg.startsWith("-")
        && CommandLine.this.commandSpec.posixOptionsMap().containsKey(arg.charAt(1)));
  }
  ...
  private Object tryConvert(ArgSpec argSpec, int index, ITypeConverter<?> converter, String value, Class<?> type)
    ...
    try {
      ...
      String msg = String.format("Invalid value for %s: %s", this.optionDescription("", argSpec, index),
          ex.getMessage());
      throw new ParameterException(CommandLine.this, msg, argSpec, value);
    } catch (Exception other) {
      String desc = this.optionDescription("", argSpec, index);
      String msg = String.format("Invalid value for %s: cannot convert '%s' to %s (%s)", desc, value,
          type.getSimpleName(), other);
  ...
  private boolean is(ArgSpec p, String attribute, boolean value) {
    if (value) {
      if (CommandLine.this.tracer.isInfo()) {
        CommandLine.this.tracer.info("%s has '%s' annotation: not validating required fields%n", p.toString(),
            attribute);
      }
    }
    return value;
  ...
  private boolean assertNoMissingParameters(ArgSpec argSpec, Range arity, Stack<String> args) {
    ...
    if (arity.min > available) {
      if (arity.min == 1) {
        ...
        this.maybeThrow(
            new MissingParameterException(CommandLine.this, argSpec, this.optionDescription("", argSpec, 0) +
                " requires at least " + arity.min + " values, but none were specified."));
      } else {
        this.maybeThrow(new MissingParameterException(CommandLine.this, argSpec,
            this.optionDescription("", argSpec, 0) +
                " requires at least " + arity.min + " values, but only " + available + " were specified: "
                + reverse(args)));
      }
      return false;
    }
...
private static class BuiltIn {
  ...
  static class ISO8601TimeConverter implements ITypeConverter<Object> {
    ...
    private Object createTime(long epochMillis) {
      try {
        ...
        throw new TypeConversionException(
            "Unable to create new java.sql.Time with long value " + epochMillis + ": " + e.getMessage());
      }
    }
  ...
  static void registerIfAvailable(Map<Class<?>, ITypeConverter<?>> registry, Tracer tracer, String fqcn,
      String factoryMethodName, Class<?>... paramTypes) {
    registerIfAvailable(registry, tracer, fqcn, fqcn, factoryMethodName, paramTypes);
  }

  static void registerIfAvailable(Map<Class<?>, ITypeConverter<?>> registry, Tracer tracer, String fqcn,
      String factoryClass, String factoryMethodName, Class<?>... paramTypes) {
    if (excluded(fqcn, tracer)) {
      return;
    }
    try {
  ...
  static boolean excluded(String fqcn, Tracer tracer) {
    ...
    for (String regex : excludes) {
      if (fqcn.matches(regex)) {
        tracer.debug("BuiltIn type converter for %s is not loaded: (picocli.converters.excludes=%s)%n", fqcn,
  ...
  static class ReflectionConverter implements ITypeConverter<Object> {
    ...
    public Object convert(String s) {
      try {
        ...
        throw new TypeConversionException(
            String.format("cannot convert '%s' to %s (%s)", s, this.method.getReturnType(), e.getTargetException()));
      } catch (Exception e) {
        throw new TypeConversionException(
            String.format("cannot convert '%s' to %s (%s)", s, this.method.getReturnType(), e));
      }
    }
  }
...
public interface IHelpCommandInitializable {
   ...
   * help command. Implementors can use this to walk the command hierarchy and get
   * access to the help command's parent and sibling commands.
...
public static class Help {
  ...
  Help addSubcommand(List<String> commandNames, CommandLine commandLine) {
    ...
    this.commands.put(all.substring(1, all.length() - 1),
        new Help(commandLine.commandSpec, this.colorScheme).withCommandNames(commandNames));
    return this;
  }
  ...
  public Help addSubcommand(String commandName, Object command) {
    this.commands.put(commandName,
        new Help(CommandSpec.forAnnotatedObject(command, this.commandSpec.commandLine().factory)));
    return this;
  }
  ...
  public String synopsis(int synopsisHeadingLength) {
    if (!empty(this.commandSpec.usageMessage().customSynopsis())) {
  ...
  public String abbreviatedSynopsis() {
    ...
    for (PositionalParamSpec positionalParam : this.commandSpec.positionalParameters()) {
      if (!positionalParam.hidden()) {
        sb.append(' ').append(this.parameterLabelRenderer().renderParameterLabel(positionalParam, this.ansi(),
  ...
  public String detailedSynopsis(int synopsisHeadingLength, Comparator<OptionSpec> optionSort,
    ...
    if (clusterBooleanOptions) { // cluster all short boolean options into a single string
      ...
      if (clusteredOptional.length() > 1) { // initial length was 1
        optionText = optionText.concat(" [").concat(this.colorScheme.optionText(clusteredOptional.toString()))
            .concat("]");
      }
    }
    for (OptionSpec option : options) {
      if (!option.hidden()) {
        ...
        Text param = this.parameterLabelRenderer().renderParameterLabel(option, this.colorScheme.ansi(),
            this.colorScheme.optionParamStyles);
  ...
  public int synopsisHeadingLength() {
    String[] lines = Ansi.OFF.new Text(this.commandSpec.usageMessage().synopsisHeading()).toString()
        .split("\\r?\\n|\\r|%n", -1);
  ...
  public String optionList(Layout layout, Comparator<OptionSpec> optionSort, IParamLabelRenderer valueLabelRenderer) {
    List<OptionSpec> options = new ArrayList<OptionSpec>(this.commandSpec.options()); // options are stored in order of
                                                                                      // declaration
    if (optionSort != null) {
  ...
  public String customSynopsis(Object... params) {
    return join(this.ansi(), this.width(), this.commandSpec.usageMessage().customSynopsis(), new StringBuilder(),
        params).toString();
  }
  ...
  public String description(Object... params) {
    return join(this.ansi(), this.width(), this.commandSpec.usageMessage().description(), new StringBuilder(), params)
        .toString();
  }
  ...
  public String header(Object... params) {
    return join(this.ansi(), this.width(), this.commandSpec.usageMessage().header(), new StringBuilder(), params)
        .toString();
  }
  ...
  public String footer(Object... params) {
    return join(this.ansi(), this.width(), this.commandSpec.usageMessage().footer(), new StringBuilder(), params)
        .toString();
  }
  ...
  public String descriptionHeading(Object... params) {
    return empty(this.commandSpec.usageMessage().descriptionHeading()) ? ""
        : heading(this.ansi(), this.width(),
  ...
  public String parameterListHeading(Object... params) {
    return this.commandSpec.positionalParameters().isEmpty() ? ""
        : heading(this.ansi(), this.width(),
  ...
  public String optionListHeading(Object... params) {
    return this.commandSpec.optionsMap().isEmpty() ? ""
        : heading(this.ansi(), this.width(),
  ...
  public String commandListHeading(Object... params) {
    return this.commands.isEmpty() ? ""
        : heading(this.ansi(), this.width(), this.commandSpec.usageMessage().commandListHeading(), params);
  }
  ...
  public String commandList() {
    ...
    for (Map.Entry<String, Help> entry : this.commands.entrySet()) {
      ...
      String header = command.usageMessage().header() != null && command.usageMessage().header().length > 0
          ? command.usageMessage().header()[0]
          : (command.usageMessage().description() != null && command.usageMessage().description().length > 0
  ...
  private static void addTrailingDefaultLine(List<Text[]> result, ArgSpec arg, ColorScheme scheme) {
    Text EMPTY = Ansi.EMPTY_TEXT;
    result.add(
        new Text[] { EMPTY, EMPTY, EMPTY, EMPTY, scheme.ansi().new Text("  Default: " + arg.defaultValueString()) });
  }
  ...
  public static class TextTable {
    ...
    private int unindent(Text[] values) {
      if (this.columns.length <= LONG_OPTION_COLUMN) {
        return 0;
      }
...
public enum Ansi
{
  ...
  ON,/** Forced OFF: never emit ANSI escape code regardless of the platform. */
  OFF;
  ...
  private static boolean ansiPossible() {
    return (isTTY() && (!isWindows || isXterm || hasOsType)) || isJansiConsoleInstalled();
  }
   ...
   * {@code "picocli.ansi"} is defined then return its boolean value, otherwise
   * return whether the platform supports ANSI escape codes
   */
...
public enum Style implements IStyle
{
  ...
  public String on() {
    return CSI + this.startCode + "m";
  }

  public String off() {
    return CSI + this.endCode + "m";
  }
   ...
   * or {@code "fg_blue"}, or {@code "46"} (indexed color) or {@code "0;5;0"} (RGB
   ...
   *
   * @param str the case-insensitive style markup to convert, e.g. {@code "blue"}
   * or {@code "bg_blue"}, or {@code "46"} (indexed color) or {@code "0;5;0"} (RGB
   * components of an indexed color)
   * @return the IStyle for the specified converter
   */
...
static class Palette256Color implements IStyle {
  ...
  public String on() {
    return String.format(CSI + "%d;5;%dm", this.fgbg, this.color);
  }

  public String off() {
    return CSI + (this.fgbg + 1) + "m";
  }
}
...
public static class UnmatchedArgumentException extends ParameterException {
  ...
  public boolean isUnknownOption() {
    return isUnknownOption(this.unmatched, this.getCommandLine());
  }
